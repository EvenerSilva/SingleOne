...
namespace SingleOneAPI.Services.TinOne
{
    ...
    public class TinOneConfigService : ITinOneConfigService
    {
        ...
        public List<TinOneConfigItemDTO> GetAllConfigs(int? clienteId = null)
        {
            ...
            {
                _logger.LogError(dbEx, "[TinOne] Erro de banco de dados ao buscar configurações - tabela pode não existir");
                return new List<TinOneConfigItemDTO>();
            }
            catch (InvalidOperationException invEx)
            {
                _logger.LogError(invEx, "[TinOne] Erro de operação ao buscar configurações - tabela pode não estar mapeada");
                return new List<TinOneConfigItemDTO>();
            }
            catch (Exception ex)
            {
                ...
                foreach (var configDTO in configuracoes)
                {
                    ...
                    if (configDTO.Id.HasValue && configDTO.Id.Value > 0)
                    {
                        ...
                        if (configExistente != null)
                        {
                            ...
                            // NÃO atualiza CreatedAt (mantém o valor original do banco)
                            configExistente.Valor = configDTO.Valor;
                            configExistente.Descricao = configDTO.Descricao;
                            configExistente.Ativo = configDTO.Ativo;
                            configExistente.UpdatedAt = DateTime.Now;
                            
                            // Marca apenas os campos específicos como modificados, não a entidade inteira
                            _context.Entry(configExistente).Property(x => x.Valor).IsModified = true;
                            _context.Entry(configExistente).Property(x => x.Descricao).IsModified = true;
                            _context.Entry(configExistente).Property(x => x.Ativo).IsModified = true;
                            _context.Entry(configExistente).Property(x => x.UpdatedAt).IsModified = true;
                            
                            // Garante que CreatedAt NÃO seja modificado
                            _context.Entry(configExistente).Property(x => x.CreatedAt).IsModified = false;
