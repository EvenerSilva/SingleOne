-- Script para corrigir a view equipamentovm
-- ================================================

-- 1. Dropar a view atual
DROP VIEW IF EXISTS equipamentovm;

-- 2. Recriar a view com os dados de navegação corretos
CREATE VIEW equipamentovm AS
SELECT 
    e.id,
    e.tipoequipamento AS tipoequipamentoid,
    COALESCE(te.descricao, 'Não definido'::character varying) AS tipoequipamento,
    e.fabricante AS fabricanteid,
    COALESCE(f.descricao, 'Não definido'::character varying) AS fabricante,
    e.modelo AS modeloid,
    COALESCE(m.descricao, 'Não definido'::character varying) AS modelo,
    e.notafiscal AS notafiscalid,
    e.equipamentostatus AS equipamentostatusid,
    COALESCE(es.descricao, 'Não definido'::character varying) AS equipamentostatus,
    e.usuario AS usuarioid,
    COALESCE(u.nome, 'Não definido'::character varying) AS usuario,
    e.localizacao AS localizacaoid,
    COALESCE(l.descricao, 'Não definido'::character varying) AS localizacao,
    e.possuibo,
    e.descricaobo,
    e.numeroserie,
    e.patrimonio,
    ...
    COALESCE(emp.nome, 'Não definido'::character varying) AS empresa,
    e.centrocusto AS centrocustoid,
    COALESCE(cc.nome, 'Não definido'::character varying) AS centrocusto,
    e.contrato AS contratoid,
    'Não definido'::character varying AS contrato
FROM equipamentos e
