-- Corrigir localidade padrão na view (versão final)
-- ================================================

-- 1. Dropar a view atual
DROP VIEW IF EXISTS equipamentovm;

-- 2. Recriar a view sem mostrar "Padrão" (usando ID em vez de texto)
CREATE VIEW equipamentovm AS
SELECT 
    e.id,
    e.tipoequipamento AS tipoequipamentoid,
    COALESCE(te.descricao, 'Nao definido') AS tipoequipamento,
    e.fabricante AS fabricanteid,
    COALESCE(f.descricao, 'Nao definido') AS fabricante,
    e.modelo AS modeloid,
    COALESCE(m.descricao, 'Nao definido') AS modelo,
    e.notafiscal AS notafiscalid,
    e.equipamentostatus AS equipamentostatusid,
    COALESCE(es.descricao, 'Nao definido') AS equipamentostatus,
    e.usuario AS usuarioid,
    COALESCE(u.nome, 'Nao definido') AS usuario,
    e.localizacao AS localizacaoid,
    CASE 
        WHEN e.localizacao = 1 THEN 'Nao definido'  -- ID 1 = Padrão
        ELSE COALESCE(l.descricao, 'Nao definido')
    END AS localizacao,
    e.possuibo,
    e.descricaobo,
    e.numeroserie,
    ...
    COALESCE(emp.nome, 'Nao definido') AS empresa,
    e.centrocusto AS centrocustoid,
    COALESCE(cc.nome, 'Nao definido') AS centrocusto,
    e.contrato AS contratoid,
    'Nao definido' AS contrato
FROM equipamentos e
