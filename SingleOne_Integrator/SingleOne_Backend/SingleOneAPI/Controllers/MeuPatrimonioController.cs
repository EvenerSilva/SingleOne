    ...
    public class MeuPatrimonioController : ControllerBase
    {
        ...
        public async Task<ActionResult<MeuPatrimonioResponseDTO>> Autenticar([FromBody] MeuPatrimonioAuthDTO authData)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] ⚠️ Relacionamentos não carregados automaticamente, tentando carregar manualmente...");
        ...
        public ActionResult<object> CriarContestacao([FromBody] CriarContestacaoDTO contestacao)
        {
            try
            {
                Console.WriteLine($"[MEU_PATRIMONIO] === CRIAR CONTESTAÇÃO ===");
                Console.WriteLine($"[MEU_PATRIMONIO] Dados recebidos: {Newtonsoft.Json.JsonConvert.SerializeObject(contestacao, Newtonsoft.Json.Formatting.Indented)}");
                Console.WriteLine($"[MEU_PATRIMONIO] ColaboradorId: {contestacao?.ColaboradorId}");
                Console.WriteLine($"[MEU_PATRIMONIO] EquipamentoId: {contestacao?.EquipamentoId}");
                Console.WriteLine($"[MEU_PATRIMONIO] Motivo: {contestacao?.Motivo}");
                Console.WriteLine($"[MEU_PATRIMONIO] Descrição: {contestacao?.Descricao}");
                Console.WriteLine($"[MEU_PATRIMONIO] EvidenciaUrl: {contestacao?.EvidenciaUrl}");

                // Validações básicas
                if (contestacao == null)
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] ERRO: Dados nulos");
                    return BadRequest(new { sucesso = false, mensagem = "Dados da contestação não foram fornecidos" });
                }

                if (contestacao.ColaboradorId <= 0)
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] ERRO: ColaboradorId inválido");
                    return BadRequest(new { sucesso = false, mensagem = "ID do colaborador é obrigatório" });
                }

                if (contestacao.EquipamentoId <= 0)
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] ERRO: EquipamentoId inválido");
                    return BadRequest(new { sucesso = false, mensagem = "ID do equipamento é obrigatório" });
                }

                if (string.IsNullOrEmpty(contestacao.Motivo))
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] ERRO: Motivo vazio");
                    return BadRequest(new { sucesso = false, mensagem = "Motivo é obrigatório" });
                }

                if (string.IsNullOrEmpty(contestacao.Descricao))
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] ERRO: Descrição vazia");
                ...
                if (sucesso)
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] Contestação criada com sucesso");
                    return Ok(new { sucesso = true, mensagem = "Contestação criada com sucesso" });
                }
                else
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] Erro ao criar contestação");
                    return BadRequest(new { sucesso = false, mensagem = "Erro ao criar contestação" });
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MEU_PATRIMONIO] ERRO: {ex.Message}");
        ...
        public ActionResult<object> CancelarContestacao([FromBody] CancelarContestacaoDTO dto)
        {
            try
            {
                Console.WriteLine($"[MEU_PATRIMONIO] === CANCELAR CONTESTAÇÃO ===");
                Console.WriteLine($"[MEU_PATRIMONIO] Dados recebidos: {Newtonsoft.Json.JsonConvert.SerializeObject(dto, Newtonsoft.Json.Formatting.Indented)}");
        ...
        public async Task<IActionResult> CriarAutoInventario([FromBody] CriarAutoInventarioDTO dados)
        {
            try
            {
                Console.WriteLine($"[MEU_PATRIMONIO] Criando Auto Inventário para colaborador {dados.ColaboradorId}");
                Console.WriteLine($"[MEU_PATRIMONIO] Número de série: {dados.NumeroSerie}");
            ...
            {
                Console.WriteLine($"[MEU_PATRIMONIO] Erro ao criar Auto Inventário: {ex.Message}");
                return StatusCode(500, new { sucesso = false, mensagem = "Erro interno do servidor" });
            }
        }
        ...
        private async Task<List<MeuPatrimonioEquipamentoDTO>> BuscarEquipamentosColaborador(int colaboradorId)
        {
            ...
            Console.WriteLine($"[MEU_PATRIMONIO] Buscando histórico (entregas/devoluções) para colaborador ID: {colaboradorId}");
            ...
            Console.WriteLine($"[MEU_PATRIMONIO] Itens considerados: {ultimosItensPorEquipamento.Count}");
            Console.WriteLine($"[MEU_PATRIMONIO] Linhas telefônicas consideradas: {ultimosItensPorLinha.Count}");
            ...
            foreach (var item in ultimosItensPorEquipamento)
            {
                ...
                Console.WriteLine($"[MEU_PATRIMONIO] Equipamento {equipamento.Id} - Item Requisicao: {item.Requisicao}");
                Console.WriteLine($"[MEU_PATRIMONIO] Requisição encontrada: {requisicao?.Id}, Hash: {requisicao?.Hashrequisicao}, Assinado: {requisicao?.Assinaturaeletronica}");

                // Se não encontrou pela ID, tentar buscar requisições do colaborador que contenham este equipamento
                if (requisicao == null)
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] Requisição {item.Requisicao} não encontrada, buscando alternativas para equipamento {equipamento.Id}");
                    ...
                    if (requisicoesColaborador != null)
                    {
                        ...
                        Console.WriteLine($"[MEU_PATRIMONIO] Encontrada requisição alternativa {requisicao.Id} para equipamento {equipamento.Id}");
                    }
                    else
                    {
                        Console.WriteLine($"[MEU_PATRIMONIO] Nenhuma requisição encontrada para equipamento {equipamento.Id}");
                    }
                }
                ...
                Console.WriteLine($"[MEU_PATRIMONIO] Equipamento {equipamento.Id}: Total contestações = {todasContestoes.Count}, Contestação pendente = {(contestacaoPendente != null ? "SIM" : "NÃO")}");
                
                // DEBUG: Listar todas as contestações para debug
                foreach (var c in todasContestoes)
                {
                    Console.WriteLine($"[DEBUG] Contestação ID: {c.Id}, Equipamento: {c.EquipamentoId}, Status: {c.Status}, Colaborador: {c.ColaboradorId}");
                }
            ...
            foreach (var item in ultimosItensPorLinha)
            {
                ...
                Console.WriteLine($"[MEU_PATRIMONIO] Linha {linha.Id} - Item Requisicao: {item.Requisicao}");
                Console.WriteLine($"[MEU_PATRIMONIO] Requisição encontrada: {requisicao?.Id}, Hash: {requisicao?.Hashrequisicao}, Assinado: {requisicao?.Assinaturaeletronica}");

                // Se não encontrou pela ID, tentar buscar requisições do colaborador que contenham esta linha
                if (requisicao == null)
                {
                    Console.WriteLine($"[MEU_PATRIMONIO] Requisição {item.Requisicao} não encontrada, buscando alternativas para linha {linha.Id}");
                    ...
                    {
                        requisicao = requisicoesColaborador;
                        Console.WriteLine($"[MEU_PATRIMONIO] Encontrada requisição alternativa {requisicao.Id} para linha {linha.Id}");
                    }
                    else
                    {
                        Console.WriteLine($"[MEU_PATRIMONIO] Nenhuma requisição encontrada para linha {linha.Id}");
                    }
                }
                ...
                Console.WriteLine($"[MEU_PATRIMONIO] Linha {linha.Id}: Total contestações = {todasContestoes.Count}, Contestação pendente = {(contestacaoPendenteLinha != null ? "SIM" : "NÃO")}");
            ...
            Console.WriteLine($"[MEU_PATRIMONIO] Total de equipamentos encontrados (histórico): {ordenados.Count}");
        ...
        public ActionResult DebugUsuarios()
        {
            ...
            {
                Console.WriteLine($"[DEBUG] Erro ao listar usuários: {ex.Message}");
                return StatusCode(500, new { sucesso = false, mensagem = ex.Message });
            }
        }
        ...
        public ActionResult DebugColaborador(int colaboradorId)
        {
            try
            {
                Console.WriteLine($"[DEBUG] Verificando colaborador ID: {colaboradorId}");
            ...
            {
                Console.WriteLine($"[DEBUG] Erro ao verificar colaborador: {ex.Message}");
                return StatusCode(500, new { sucesso = false, mensagem = ex.Message });
            }
        }
        ...
        public ActionResult GerarTermoPDF(int equipamentoId, [FromQuery] int? colaboradorId = null)
        {
            try
            {
                Console.WriteLine($"[MEU_PATRIMONIO] === GERAR PDF DO TERMO ===");
                Console.WriteLine($"[MEU_PATRIMONIO] Equipamento ID: {equipamentoId}, Colaborador ID: {colaboradorId}");
                Console.WriteLine($"[MEU_PATRIMONIO] Repositório requisicaoequipamentosvms: {(_requisicaoequipamentosvmsRepository == null ? "NULL ❌" : "OK ✅")}");
        ...
        public ActionResult<object> ObterContestoesColaborador(int colaboradorId)
        {
            try
            {
                ...
                // Buscar contestações reais da tabela
                var contestoes = _contestacaoRepository.Buscar(x => x.ColaboradorId == colaboradorId)
                ...
                foreach (var c in contestoes)
                {
                    ...
                    var equipamento = _equipamentoRepository.Buscar(e => e.Id == c.EquipamentoId).FirstOrDefault();
                    ...
                    resultado.Add(new
                    {
                        ...
                        tecnicoResponsavel = c.UsuarioResolucaoNavigation?.Nome ?? "Não atribuído",
                        ...
                        {
                            id = c.EquipamentoId,
                            nome = equipamento?.Patrimonio ?? "Equipamento",
                            numeroSerie = equipamento?.Numeroserie ?? "N/A",
                            tipoEquipamento = equipamento?.TipoequipamentoNavigation?.Descricao ?? "Patrimônio"
                        },
                        colaborador = new
                        {
                            id = c.ColaboradorId,
