...
namespace SingleOne.Controllers
{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        private static readonly Dictionary<int, string> _twoFactorCodes = new Dictionary<int, string>();
        
        // Método para limpar códigos expirados (opcional)
        private void CleanupExpiredCodes()
        {
            // Em produção, você pode implementar um timer para limpar códigos antigos
        ...
        public async Task<ActionResult<dynamic>> Login(Usuario usr)
        {
            try
            {
                Console.WriteLine($"[CONTROLLER] === INÍCIO DO LOGIN ===");
                Console.WriteLine($"[CONTROLLER] Email recebido: {usr.Email}");
                Console.WriteLine($"[CONTROLLER] Senha recebida: {usr.Senha}");
                
                var usuario = _negocio.Logar(usr);
                
                if (usuario != null)
                {
                    Console.WriteLine($"[CONTROLLER] ✓ Usuário encontrado: {usuario.Nome}");
                    ...
                    {
                        Console.WriteLine($"[CONTROLLER] 2FA necessário para usuário: {usuario.Nome}");
                        ...
                        Console.WriteLine($"[CONTROLLER] ✓ Retornando resposta com 2FA necessário");
                        Console.WriteLine($"[CONTROLLER] === FIM DO LOGIN (2FA NECESSÁRIO) ===");
                        return Ok(response);
                    }
                    else
                    {
                        Console.WriteLine($"[CONTROLLER] 2FA não necessário para usuário: {usuario.Nome}");
                        ...
                        Console.WriteLine($"[CONTROLLER] ✓ Retornando resposta OK");
                        Console.WriteLine($"[CONTROLLER] === FIM DO LOGIN (SUCESSO) ===");
                        return Ok(response);
                    }
                }
                else
                {
                    Console.WriteLine($"[CONTROLLER] ✗ Usuário não encontrado, retornando 401");
                    Console.WriteLine($"[CONTROLLER] === FIM DO LOGIN (FALHA) ===");
                    return Unauthorized(new { Mensagem = "Usuário/Senha não encontrado.", Status = "401" });
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[CONTROLLER] ERRO: {ex.Message}");
        ...
        public async Task<ActionResult<dynamic>> VerifyTwoFactor(TwoFactorVerificationRequest request)
        {
            try
            {
                Console.WriteLine($"[CONTROLLER] === VERIFICAÇÃO 2FA ===");
                Console.WriteLine($"[CONTROLLER] Usuário ID: {request.UserId}");
                Console.WriteLine($"[CONTROLLER] Tipo de verificação: {request.VerificationType}");
                ...
                {
                    // Verificar código de email usando o código armazenado em memória
                    if (_twoFactorCodes.TryGetValue(request.UserId, out string storedCode))
                    {
                        isValid = storedCode == request.Code;
                        Console.WriteLine($"[CONTROLLER] 🔍 Código armazenado: {storedCode}, código recebido: {request.Code}, válido: {isValid}");
                        
                        // Remover o código após verificação (usado apenas uma vez)
                        if (isValid)
                        {
                            _twoFactorCodes.Remove(request.UserId);
                            Console.WriteLine($"[CONTROLLER] ✅ Código 2FA removido da memória após verificação bem-sucedida");
                        }
                    }
                    else
                    {
                        Console.WriteLine($"[CONTROLLER] ❌ Código 2FA não encontrado na memória para usuário: {request.UserId}");
                        isValid = false;
                    }
                }
                ...
                if (isValid)
                {
                    Console.WriteLine($"[CONTROLLER] ✓ Verificação 2FA bem-sucedida");
                    
                    var usuario = _negocio.BuscarPorId(request.UserId);
                    var token = JwtTokenService.GenerateToken(usuario);
                    
                    // Atualizar último uso do 2FA
                    usuario.TwoFactorLastUsed = DateTime.UtcNow;
                    ...
                    Console.WriteLine($"[CONTROLLER] 🔍 Resposta sendo retornada:");
                    Console.WriteLine($"[CONTROLLER] 🔍 - success: {response.success}");
                    Console.WriteLine($"[CONTROLLER] 🔍 - message: {response.message}");
                    Console.WriteLine($"[CONTROLLER] 🔍 - usuario.id: {response.usuario.id}");
                    Console.WriteLine($"[CONTROLLER] 🔍 - token: {(string.IsNullOrEmpty(response.token) ? "NULL" : "PRESENTE")}");
                    Console.WriteLine($"[CONTROLLER] === FIM VERIFICAÇÃO 2FA (SUCESSO) ===");
                    return Ok(response);
                }
                else
                {
                    Console.WriteLine($"[CONTROLLER] ✗ Verificação 2FA falhou");
                    Console.WriteLine($"[CONTROLLER] === FIM VERIFICAÇÃO 2FA (FALHA) ===");
            ...
            {
                Console.WriteLine($"[CONTROLLER] ERRO na verificação 2FA: {ex.Message}");
        ...
        {
            Console.WriteLine($"[CONTROLLER] 🔥🔥🔥 MÉTODO SendTwoFactorCode CHAMADO! 🔥🔥🔥");
            Console.WriteLine($"[CONTROLLER] 🔥🔥🔥 Request completo: {System.Text.Json.JsonSerializer.Serialize(request)} 🔥🔥🔥");
            
            try
            {
                Console.WriteLine($"[CONTROLLER] === INÍCIO ENVIO CÓDIGO 2FA ===");
                Console.WriteLine($"[CONTROLLER] Usuário ID: {request.UserId}");
                Console.WriteLine($"[CONTROLLER] Email: {request.Email}");
                
                // Gerar código de 6 dígitos
                var random = new Random();
                var code = random.Next(100000, 999999).ToString();
                
                Console.WriteLine($"[CONTROLLER] Código gerado: {code}");
                
                // Armazenar código temporariamente para verificação
                _twoFactorCodes[request.UserId] = code;
                Console.WriteLine($"[CONTROLLER] ✅ Código 2FA armazenado temporariamente: {code} para usuário: {request.UserId}");
                
                // Enviar código por e-mail
                Console.WriteLine($"[CONTROLLER] 🔍 Enviando código por email: {code} para {request.Email}");
                var resultado = await _negocio.EnviarCodigoTwoFactor(request.Email, code);
                
                if (resultado)
                {
                    Console.WriteLine($"[CONTROLLER] ✅ Código 2FA enviado com sucesso para: {request.Email}");
                    Console.WriteLine($"[CONTROLLER] === FIM ENVIO CÓDIGO 2FA (SUCESSO) ===");
                ...
                {
                    Console.WriteLine($"[CONTROLLER] ❌ Falha ao enviar código 2FA para: {request.Email}");
                    Console.WriteLine($"[CONTROLLER] === FIM ENVIO CÓDIGO 2FA (FALHA) ===");
            ...
            {
                Console.WriteLine($"[CONTROLLER] ERRO ao enviar código 2FA: {ex.Message}");
                return StatusCode(500, new { 
                    success = false, 
