...
namespace SingleOne.Negocios
{
    public class RequisicoesNegocio : IRequisicoesNegocio
    {
        ...
        public PagedResult<RequisicaoVM> ListarRequisicoes(string pesquisa, int cliente, int pagina)
        {
            ...
            // ✅ DEBUG: Log para verificar busca por telefone
            Console.WriteLine($"[DEBUG] Busca por telefone '{pesquisa}': {requisicoesPorTelefone.Count} requisições encontradas");
            Console.WriteLine($"[DEBUG] Linhas telefônicas encontradas: {linhasEncontradas.Count}");
            
            // ✅ DEBUG: Verificar se há itens com linha telefônica
            var itensComTelefone = _requisicaoItensRepository.Buscar(ri => ri.Linhatelefonica.HasValue).ToList();
            Console.WriteLine($"[DEBUG] Total de itens com linha telefônica: {itensComTelefone.Count}");
            
            // ✅ DEBUG: Verificar se há linhas telefônicas ativas
            var totalLinhasAtivas = _telefonialinhaRepository.Buscar(tl => tl.Ativo).Count();
            Console.WriteLine($"[DEBUG] Total de linhas telefônicas ativas: {totalLinhasAtivas}");
            
            if (itensComTelefone.Any())
            {
                Console.WriteLine($"[DEBUG] Primeiros itens com telefone: {string.Join(", ", itensComTelefone.Take(3).Select(ri => $"Req:{ri.Requisicao}, Linha:{ri.Linhatelefonica}"))}");
            }
            ...
            {
                Console.WriteLine($"[DEBUG] IDs combinados: {string.Join(", ", idsCombinados)}");
                Console.WriteLine($"[DEBUG] Buscando requisições com IDs: {string.Join(", ", idsCombinados)}");
                ...
                Console.WriteLine($"[DEBUG] Requisições encontradas após busca combinada: {vw.Results.Count}");
            }
        ...
        private List<RequisicaoColaboradorVM> MontarRequisicoesColaborador(List<UltimaRequisicaoDTO> qUltimaRequisicaoBYOD)
        {
            ...
            foreach (var item in qUltimaRequisicaoBYOD)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"[ERROR] MontarRequisicoesColaborador - Erro ao processar requisição {item.RequisicaoId}: {ex.Message}");
                }
            }
            return requisicoes;
        }
        ...
        public void AtualizarItemRequisicao(Requisicaoequipamentosvm rivm)
        {
            try
            {
                ...
                //ri.Dtprogramadaretorno = rivm.Dtprogramadaretorno.Value.Date;
                ri.Dtprogramadaretorno = rivm.Dtprogramadaretorno;
                //db.Update(ri);
                //db.SaveChanges();
        ...
        public void AdicionarAgendamentoEquipamentoVM(EquipamentoRequisicaoVM equipamento)
        {
            var item = _requisicaoItensRepository
                ...
                .AsNoTracking()
                .FirstOrDefault();
            item.Dtprogramadaretorno = equipamento.DTProgramadaRetorno;
            _requisicaoItensRepository.Atualizar(item);
        }
